...
public class TimeSheetLineDAO 
{
	...
	public static int  updateTimeSheetLine(String columnname, String fromvalue, String tovalue) {
		Query query = EM.getEM().createQuery("UPDATE " + TimeSheetLine.class.getName() + " SET " + 
	...
	public static ArrayList<TimeSheetLine> listTimeSheetLines() {
		Query query = EM.getEM().createQuery("SELECT project FROM " + TimeSheetLine.class.getName() + " project", TimeSheetLine.class);
	...
	public static TimeSheetLine findTimeSheetLineById(long id) {
		TimeSheetLine timesheetline = EM.getEM().find(TimeSheetLine.class, new Long(id));
