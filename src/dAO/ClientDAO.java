...
public class ClientDAO {
	...
	public static int  updateClient(String columnname, String fromvalue, String tovalue) {
		Query query = EM.INSTANCE.getEM().createQuery("UPDATE " + Client.class.getName() + " SET " + 
	...
	public static ArrayList<Client> listClients() {
		Query query = EM.INSTANCE.getEM().createQuery("SELECT client FROM "+ Client.class.getName() + " client", Client.class);
	...
	public static Client findClientById(long id) {
		Client client = EM.INSTANCE.getEM().find(Client.class, new Long(id));
		return client;
	}
	...
	public static void removeClientByID(long id) {
		...
		if ( client != null){
			EM.INSTANCE.getEM().remove(client);
			return;
		}
